...
Require all nmp packages - (inside the server.js folder)
Set body parser
Set local host port
Establish mongo db connection // use mongoose in my server 

set up codes for logger(morgan)
express
require models

Set up React - //
Set up Express (will help me with my routes)  (we can create the routes - look at a previous class example - it will look similar to previous hw) 

Set up the components / look at the example pictures
	1- Search
	2- Results
	3- Saved Article 
Will need a component folder / every will be be a separate file ()
Then in the app.js is where I will be using all together.
NPM AND  YARN - ARE NOT - NOT - NOT SUPPOSSE TO GO TOGETHER…
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=

connect to rest of the ROUTES:
use an example of scrapping (line 41) 18.11

look at the elements of the page / look at paterns

visit the example / look at the console and look at the elements... 

inspect the page and find the pattern in the elements.
example: titles / url / image source (store)
...
//imput field for year (google it)
